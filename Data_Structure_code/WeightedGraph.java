...
public class WeightedGraph<V extends Number> extends AbstractGraph<V> {
    ...
    /** Construct a WeightedGraph from vertices, edged and weights in arrays */
    public WeightedGraph(V[] vertices, int[][] edges, Double[][] weights) {
        createWeightedGraph(java.util.Arrays.asList(vertices), edges, weights);
    }

    /** Construct a WeightedGraph from vertices, edged and weights in list */
    ...
    private void createWeightedGraph(List<V> vertices, int[][] edges, Double[][] weights) {
        ...
        for (int[] edge : edges) {
            if(weights[edge[0]][edge[1]] == null){
                continue;
            }
            neighbors.get(edge[0]).add(new WeightedEdge(edge[0], edge[1],weights[edge[0]][edge[1]]));
        }
    }
    ...
    /** Get Prim's Minimum Spanning Tree rooted at a specified vertex */
